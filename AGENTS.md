...
- **External deps are optional at import:** All external Python libs (`duckduckgo-search`, `edge-tts`, `vosk`) are wrapped in `try/except ImportError` blocks. The module installs and loads without them, but features fail gracefully at runtime.
